function [latbp, lonbp, grid, windowSize] = def_vertical(model, nmax, varargin)
...
if exist('functional' , 'var')
   if strcmpi(functional , 'psi')
      n_functional   = 1;
      label          = 'East - West Components psi';
      elseif strcmpi('functional', 'eta')
      n_functional = 2;
      label = 'North - South Components eta'
...
temp_folder          = [pathstr '/deflection_vertical/'];
if ~exist(temp_folder, 'file')
   mkdir(temp_folder);
   fid               = fopen([temp_folder 'readme.txt'] , 'w');

   fprintf(fid,'%% This folder was created by function: def_vertical.m\n');
   fprintf(fid,'%% In the folder ''deflection_vertical'' the computed geopotential grids are stored for speeding up the execution of the function.\n');
...
latbp             = latbp - 90;
lat               = latbp/rad;
sin_mlat          = sin((0:nmax)'*lat);

%Or even nicer we may do a nested function here.

%% Computation of r,
e2 = 0.00669437999013;
N = Re./sqrt(1-e2*sin(latbp).^2);
X = (N.*cos(latbp))'*cos(lonbp);
Y = (N.*cos(latbp))'*sin(lonbp);
Z = ((N.*(1-e2)))'*sin(latbp);
r = sqrt(X.^2+Y.^2+Z.^2);


if n_functional      == 1
   grid       = GM/(Re*r*sin_mlat) * (Am*cos_mlon + Bm*sin_mlon);
